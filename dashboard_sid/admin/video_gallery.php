...
<?php include("right_sidebar_backdrop.php"); 
	...
	if (isset($_POST['delete']))
	{
		...
		$id = $_POST['id'];
			$del_qry = "update video_gallery set status = 1 where id = $id";
    	$del_res = mysqli_query($dblink,$del_qry);
    	if($del_res){
    		 echo '<script> success_message("Success","success","Video deleted successfully","video_gallery.php"); </script>';
    	}
    	else{
	...
	{
		$id = $_POST['id'];
		list($video) = exc_qry("select * from video_gallery where id = $id");
		$link = $video[0]['link'];
		$cat_id = $video[0]['cat_id'];
	...
	if(isset($_POST['add'])){
		...
		$link = $_POST['link'];
		$ins_qry = "insert into video_gallery set link = '$link',cat_id = $cat_id , add_date = now()";
	    	$ins_res = mysqli_query($dblink,$ins_qry);
	    	if($ins_res){
	    		 echo '<script> success_message("Success","success","video added successfully","video_gallery.php"); </script>';
	    	}
	    	else{
	...
	if(isset($_POST['update'])){
		...
		$cat_id = $_POST['category'];
		$link = $_POST['link'];
	    	$upd_qry = "update video_gallery set cat_id = $cat_id, edit_date = now(), link='$link' where id = $id";
	    	$upd_res = mysqli_query($dblink,$upd_qry);
	    	if($upd_res){
	    		 echo '<script> success_message("Success","success","video edited successfully","video_gallery.php"); </script>';
	    	}
	    	else{
	    		$msg = mysqli_error($dblink);
...
<!-- Main Content -->
						...
						<div class="col-md-12">
							<div class="panel panel-default card-view">
								<div class="panel-heading">
									<div class="pull-left">
										<h6 class="panel-title txt-dark">Video</h6>
									</div>
												...
												<div class="form-wrap">
													<form method="post">
														<div class="form-group">
															<label class="control-label mb-10" for="exampleInputuname_1"> Embed link *</label>
															<input type="hidden" name="id" id="input1" value="<?php echo $id; ?>">
															<div class="input-group">
																<div class="input-group-addon"><i class="icon-user"></i></div>
																<input type="url" class="form-control" id="input2" placeholder="link" name="link" placeholder="Icon class name" required  value="<?php echo $link; ?>">
															</div>
														</div>
														...
														<button class="btn  btn-danger ml-10" type="button" onclick="window.location='video_gallery.php'">Cancel</button>	
														<?php } else { ?>
			...
			<div  class="row class="card-view"">
					<?php list($gallery) = exc_qry("select * from video_gallery where status = 0 order by id desc");
										//echo count($slider);
						...
						<div class="panel panel-default card-view">
							...
							<div class="panel-wrapper collapse in row">
								<div class="panel-body col-sm-12">
									<div class="ml-20">
										<iframe src="<?php echo $gallery[$i]['link']?>"  allow="autoplay; encrypted-media" allowfullscreen></iframe>
									</div>	
								</div>
							</div>
						</div>
